const ALL_TOOLS = [
  { name: "BriefOS", desc: "AI brief generator", url: "https://briefos-silk.vercel.app", live: true },
  ...
  { name: "DayForge", desc: "AI day planner", url: null, live: false },
  { name: "MeetingMind", desc: "Meeting notes → actions", url: null, live: false },
  { name: "ContractLens", desc: "Contract risk scanner", url: null, live: false },
];

const CURRENT = "InvoiceKit";

export default function Footer() {
  const others = ALL_TOOLS.filter((t) => t.name !== CURRENT);

  return (
    <footer className="border-t border-border mt-8 px-4 py-16 no-print">
      <div className="mx-auto max-w-4xl">
          ...
          {others.map((t) =>
            t.live && t.url ? (
              <a
                ...
                className="rounded-xl border border-border bg-surface p-3 hover:border-accent/40 transition-colors group"
              >
        ...
        <p className="mt-10 text-center text-xs text-muted/50">
          InvoiceKit · Built by{" "}
          <a href="https://arknet.digital" target="_blank" rel="noopener noreferrer" className="hover:text-muted transition-colors">
            ArkNet Digital
          </a>
        </p>
      </div>
